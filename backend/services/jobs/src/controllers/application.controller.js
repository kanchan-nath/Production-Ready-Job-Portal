...
const createApplication = asyncHandler(async(req, res) => {
    ...
    // Validation
    if (!jobId || !fullName || !email || !phone || !resume || !coverLetter) {
    ...
    if (job.deadline && today > new Date(job.deadline)) {
        return res.status(400).json(new ApiResponse(400, null, "Application deadline has passed"));
    }
    ...
    return res.status(201).json(new ApiResponse(201, savedApplication, "Application submitted successfully"));
});
